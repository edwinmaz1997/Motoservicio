...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $db   = getDB();
        $stmt = $db->prepare("INSERT INTO clientes (sucursal_id, nombre, apellido, direccion, dpi, nit, telefono, correo)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $user['sucursal_id'],
            $data['nombre'], $data['apellido'], $data['direccion'],
            $data['dpi'], $data['nit'], $data['telefono'], $data['correo']
        ]);
    ...
    <form method="POST">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="dpi" value="<?= sanitize($data['dpi']) ?>"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">NIT</label>
                <input type="text" name="nit" value="<?= sanitize($data['nit']) ?>"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
        ...
        <div class="flex gap-3 mt-6">
            ...
            <a href="index.php" class="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-200 transition text-sm">
                Cancelar
            </a>
        </div>
    </form>
