...
$stmt = $db->prepare("
    SELECT o.*,
           ...
           COALESCE(CONCAT(a.nombre,' ',a.apellido),'—') as asesor,
...
$anticipos = $db->prepare("SELECT an.*, COALESCE(CONCAT(u.nombre,' ',u.apellido),'—') as cajero FROM anticipos an LEFT JOIN usuarios u ON u.id=an.cajero_id WHERE an.orden_id=? ORDER BY an.created_at");
...
$pageTitle = 'Orden ' . $o['numero_orden'];
...
<div class="mb-5">
    <div class="flex items-center gap-2 mb-3">
        ...
        <h1 class="text-lg lg:text-2xl font-bold text-gray-800"><?= sanitize($o['numero_orden']) ?></h1>
    ...
    <div class="flex gap-2 overflow-x-auto pb-1">
        ...
        <a href="cambiar_estado.php?id=<?= $o['id'] ?>&estado=<?= $siguienteEstado[$o['estado']] ?>"
           class="flex-shrink-0 bg-green-600 text-white px-3 py-2 rounded-lg text-sm hover:bg-green-700 whitespace-nowrap"
        ...
        <a href="edit.php?id=<?= $o['id'] ?>"
           class="flex-shrink-0 bg-yellow-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-yellow-600 whitespace-nowrap">
            <i class="fas fa-edit mr-1"></i> Editar
        </a>
        <a href="cancelar.php?id=<?= $o['id'] ?>"
           class="flex-shrink-0 bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 whitespace-nowrap"
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <!-- Info general — cards en móvil -->
        <div class="bg-white rounded-xl shadow p-4 lg:p-6">
            <h2 class="font-semibold text-gray-700 mb-3 text-sm uppercase tracking-wide">Información General</h2>
            <div class="grid grid-cols-2 gap-3 text-sm">
                <div>
                    <span class="text-gray-400 text-xs block">Cliente</span>
                    <a href="../clientes/view.php?id=<?= $o['cliente_id'] ?>" class="font-medium text-blue-600 hover:underline"><?= sanitize($o['cliente']) ?></a>
                    <?php if ($o['cliente_tel']): ?><div class="text-gray-400 text-xs"><?= sanitize($o['cliente_tel']) ?></div><?php endif; ?>
                </div>
                <div>
                    <span class="text-gray-400 text-xs block">Motocicleta</span>
                    <span class="font-medium"><?= sanitize($o['moto']) ?></span>
                    <?php if ($o['placa']): ?><div class="text-gray-400 text-xs"><?= sanitize($o['placa']) ?></div><?php endif; ?>
                </div>
                <div>
                    <span class="text-gray-400 text-xs block">Técnico</span>
                    <span class="font-medium"><?= sanitize($o['tecnico']) ?></span>
                </div>
                <div>
                    <span class="text-gray-400 text-xs block">Asesor</span>
                    <span class="font-medium"><?= sanitize($o['asesor']) ?></span>
                </div>
                <div>
                    <span class="text-gray-400 text-xs block">Ingreso</span>
                    <span class="font-medium"><?= formatDate($o['fecha_ingreso']) ?></span>
                </div>
                <div>
                    <span class="text-gray-400 text-xs block">Salida esperada</span>
                    <span class="font-medium"><?= formatDate($o['fecha_salida_esperada']) ?: '—' ?></span>
                </div>
                <?php if ($o['fecha_salida_real']): ?>
                <div>
                    <span class="text-gray-400 text-xs block">Salida real</span>
                    <span class="font-medium"><?= formatDate($o['fecha_salida_real']) ?></span>
                </div>
                <?php endif; ?>
                <div>
                    <span class="text-gray-400 text-xs block">Kilometraje</span>
                    <span class="font-medium"><?= $o['kilometraje_ingreso'] ? number_format($o['kilometraje_ingreso']).' km' : '—' ?></span>
                </div>
            </div>
            <?php if ($o['diagnostico']): ?>
            <div class="mt-3 pt-3 border-t">
                <span class="text-gray-400 text-xs block mb-1">Diagnóstico</span>
                <p class="text-sm"><?= nl2br(sanitize($o['diagnostico'])) ?></p>
            </div>
            <?php endif; ?>
            <?php if ($o['observaciones']): ?>
            <div class="mt-2">
                <span class="text-gray-400 text-xs block mb-1">Observaciones</span>
        ...
        <div class="bg-white rounded-xl shadow p-4 lg:p-6">
            ...
            <div class="lg:hidden space-y-2">
                ...
                <p class="text-gray-400 text-sm text-center py-4">Sin items</p>
                <?php endif; ?>
                <?php foreach ($detalle as $d): ?>
                <div class="flex justify-between items-center py-2 border-b last:border-0 text-sm">
                    <div>
                        <div class="font-medium"><?= sanitize($d['producto']) ?></div>
                        <div class="text-xs text-gray-400"><?= $d['cantidad'] ?> x <?= formatMoney($d['precio_unit']) ?></div>
                    </div>
                    <div class="font-semibold"><?= formatMoney($d['subtotal']) ?></div>
                </div>
                <?php endforeach; ?>
                <div class="flex justify-between font-bold pt-2 text-sm">
                    <span>Total</span>
        ...
        <div class="bg-white rounded-xl shadow p-4 lg:p-6">
            <div class="flex items-center justify-between mb-3">
                <h2 class="font-semibold text-gray-700 text-sm uppercase tracking-wide">Cobros / Anticipos</h2>
                <?php if (!in_array($o['estado'],['entregada','cancelada'])): ?>
            ...
            <div class="lg:hidden space-y-2">
                ...
                <div class="flex justify-between items-center py-2 border-b last:border-0 text-sm">
                    <div>
                        <div class="font-medium text-green-600"><?= formatMoney($a['monto']) ?></div>
                        <div class="text-xs text-gray-400"><?= formatDate($a['created_at']) ?> · <?= sanitize($a['cajero']) ?> · <?= ucfirst($a['metodo_pago']) ?></div>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="hidden lg:block">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php if (empty($anticipos)): ?><tr><td colspan="4" class="px-3 py-4 text-center text-gray-400">Sin pagos</td></tr><?php endif; ?>
                        <?php foreach ($anticipos as $a): ?>
                        <tr>
            ...
            <!-- Resumen financiero -->
            <div class="grid grid-cols-3 gap-3 mt-4 pt-4 border-t text-center text-sm">
                <div>
                    <div class="text-gray-400 text-xs mb-1">Total</div>
                    <div class="font-bold"><?= formatMoney($o['total']) ?></div>
                </div>
                <div>
                    <div class="text-gray-400 text-xs mb-1">Cobrado</div>
                    <div class="font-bold text-green-600"><?= formatMoney($o['anticipo']) ?></div>
                </div>
                <div>
                    <div class="text-gray-400 text-xs mb-1">Saldo</div>
                    <div class="font-bold text-red-600"><?= formatMoney($o['saldo']) ?></div>
                </div>
            </div>
        </div>
    ...
    <div class="text-sm space-y-4">
        <div class="bg-white rounded-xl shadow p-4">
            <div class="text-gray-400 text-xs mb-1">Sucursal</div>
            <div class="font-medium"><?= sanitize($o['sucursal']) ?></div>
            <div class="text-gray-400 text-xs mt-3 mb-1">Creado</div>
...
<!-- Modal anticipo -->
<div id="modal-anticipo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div class="flex items-center justify-between px-6 py-4 border-b">
            <h3 class="font-semibold text-gray-800">Registrar Pago</h3>
            <button onclick="document.getElementById('modal-anticipo').classList.add('hidden')" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></button>
        </div>
        <form method="POST" action="anticipo.php" class="p-6 space-y-3">
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-blue-700 text-white px-5 py-2 rounded-lg text-sm hover:bg-blue-800 font-medium">Guardar</button>
